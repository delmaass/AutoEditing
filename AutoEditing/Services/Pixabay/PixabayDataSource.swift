...
class PixabayDataSource: ImageDataSource {
    func fetchImages(
        ...
        limit: Int,
        offset: Int,
        completion: @escaping ([Image]?, Error?) -> (Void)
    ) {
        ...
        var url = URL(
            string: "https://pixabay.com/api/?image_type=photo&key=\(apiKey)"
        )!
        url.appendQueryItem(name: "q", value: query)
        url.appendQueryItem(name: "per_page", value: limit.description)
        url
            .appendQueryItem(
                name: "page",
                value: ((offset / limit) + 1).description
            )
